    ...
    return (
      <View style={[styles.overlay, { paddingTop: insets.top }]}>
        <View style={styles.topBar}>
          ...
          <Text style={styles.topBarTitle}>Select Currency</Text>
    ...
    return (
      <View style={[styles.overlay, { paddingTop: insets.top }]}>
        <View style={styles.topBar}>
          ...
          <Text style={styles.topBarTitle}>Add Asset</Text>
          <View style={styles.topBarSpacer} />
        </View>
        <ScrollView contentContainerStyle={styles.typeGridContent}>
          <Text style={styles.stepHint}>What would you like to add?</Text>
    ...
    <View style={[styles.overlay, { paddingTop: insets.top }]}>
      ...
      >
        ...
        {type.valuationClass === 'FX' ? (
          <>
            {type.id === 'lent'
              ? textField('Who owes you? *', personName, setPersonName, 'e.g. Ahmet')
              : null}
            {currencyField('Currency', currencyCode, 'currency')}
            {numberField('Amount *', amount, setAmount, '0')}
          </>
        ) : null}
          ...
          <>
            {type.id === 'gold' ? (
              <>
                ...
                {chips(
                  [
                    { key: 'piece', text: 'Piece (Çeyrek, Tam…)' },
                    { key: 'weight', text: 'Weight (gram + karat)' },
                  ],
                  goldMode,
                ...
                {goldMode === 'piece' ? (
                  <>
                    ...
                    {chips(
                      GOLD_PIECES.map((p) => ({ key: p.id, text: p.labelTR })),
                      pieceId,
                      setPieceId
                    )}
                    {numberField('Count *', count, setCount, '1')}
                  </>
                ) : (
                  <>
                    {numberField('Weight (gram) *', weight, setWeight, '0')}
              ...
              numberField('Weight (kg) *', weight, setWeight, '0')
            ) : (
              numberField('Weight (gram) *', weight, setWeight, '0')
            )}
          </>
        ...
        {type.valuationClass === 'CRYPTO'
          ? numberField(`Amount (${type.symbol}) *`, units, setUnits, '0')
          : null}
        ...
        {type.valuationClass === 'MANUAL' ? (
          <>
            {type.id === 'other'
              ? textField('Asset name *', label, setLabel, 'e.g. Land deed, Watch collection')
              : null}
            {numberField('Estimated value *', manualValue, setManualValue, '0')}
            {currencyField('Value currency', manualCurrency, 'manual')}
          </>
        ) : null}

        {/* --- Ortak opsiyoneller --- */}
        {type.id !== 'other'
          ? textField('Name (optional)', label, setLabel, 'e.g. Wedding gold, Home safe')
          : null}

        <View style={styles.divider} />
        <Text style={styles.sectionTitle}>Purchase info (optional)</Text>
        <Text style={styles.sectionHint}>
          Enter what you paid to see gain/loss on this asset.
        </Text>
        {numberField('Purchase value', purchaseValue, setPurchaseValue, '0')}
        {parseNum(purchaseValue) > 0
          ? currencyField('Purchase currency', purchaseCurrency, 'purchase')
          : null}
      </ScrollView>
